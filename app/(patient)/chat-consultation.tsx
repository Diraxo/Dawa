...
export default function ChatConsultationScreen() {
  ...
  useEffect(() => {
    if (!channelId || !dbUserId) return
    let cancelled = false
    fetchHasBlockedPeer(supabase, channelId, dbUserId)
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      searchChannelMessages(activeChannel, trimmed, 50)
        ...
        .catch((err) => {
          logger.error('[Chat] message search failed:', err)
  ...
  useEffect(() => {
    ...
    const tryWatch = async () => {
      ...
      try {
        ...
        const ch = await watchConsultationChannel(channelId, members, { presence: true } as any)
        currentChannel = ch
        if (!mounted) return
      ...
      <Modal visible={showBlockModal} transparent animationType="fade" onRequestClose={() => setShowBlockModal(false)}>
        <View style={styles.blockOverlay}>
          <View style={styles.blockSheet}>
            ...
            <Pressable
              ...
              onPress={async () => {
                ...
                try {
                  ...
                  await blockConsultationPeer(channelId, token)
                  setIsBlocked(true)
                  setShowBlockModal(false)
                } catch (err) {
                  logger.error('[Chat] block peer failed:', err)
            ...
            >
              {blocking ? <ActivityIndicator color={colors.mistWhite} /> : <Text style={styles.blockConfirmText}>Block Doctor</Text>}
            </Pressable>
