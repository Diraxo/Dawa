...
// ClientRoleType.ClientRoleBroadcaster = 1 (hardcoded to avoid requiring the native module at load time)
  ...
  useEffect(() => {
    ...
    getToken().then(clerkToken => {
      ...
      fetchAgoraToken(channelName, localUid, clerkToken)
        .then(setAgoraToken)
        .catch((err) => logger.error('[Agora] Doctor phone token fetch failed:', err))
    })
