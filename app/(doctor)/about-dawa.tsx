  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>About Dawa</Text>
        <View style={{ width: 36 }} />
      </View>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.cardBody}>
            Dawa was built with one goal: to eliminate the barriers between patients and quality healthcare. We believe everyone — regardless of location, income, or circumstance — deserves fast access to a trusted doctor.
          </Text>
        ...
        {CORE_VALUES.map((val) => (
          <View key={val.title} style={styles.valueRow}>
            <View style={[styles.valueIcon, { backgroundColor: `${val.color}18` }]}>
              <Ionicons name={val.icon as never} size={22} color={val.color} />
            </View>
        ...
        <View style={styles.card}>
          <Text style={styles.cardTitle}>How Dawa Works for Doctors</Text>
          {[
            ...
            <View key={item.step} style={styles.stepRow}>
              <LinearGradient colors={gradients.interactive} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.stepNum}>
        ...
        <View style={styles.card}>
          ...
          {[
            ...
            <View key={item.text} style={styles.contactRow}>
              <Ionicons name={item.icon as never} size={17} color={colors.tealGreen} />
              <Text style={styles.contactText}>{item.text}</Text>
            </View>
          ))}
        </View>

        <Text style={styles.copyright}>© {new Date().getFullYear()} Dawa. All rights reserved.</Text>
...
const styles = StyleSheet.create({
  ...
  hero: { borderRadius: 20, padding: 28, alignItems: 'center', marginBottom: 20, ...shadow(colors.careBlue, 0, 4, 12, 0.22, 5) },
  heroTagline: { fontFamily: fonts.semiBold, fontSize: 12, color: 'rgba(255,255,255,0.8)', letterSpacing: 1.2, textAlign: 'center', marginBottom: 8 },
  heroVersion: { fontFamily: fonts.regular, fontSize: 12, color: 'rgba(255,255,255,0.6)' },

  card: { backgroundColor: colors.mistWhite, borderRadius: 16, padding: 18, marginBottom: 16, ...shadow('#000', 0, 1, 5, 0.05, 2) },
  ...
  statCard: { flex: 1, backgroundColor: colors.mistWhite, borderRadius: 14, paddingVertical: 14, alignItems: 'center', ...shadow('#000', 0, 1, 4, 0.04, 1) },
  ...
  sectionLabel: { fontFamily: fonts.semiBold, fontSize: 12, color: '#6B7280', textTransform: 'uppercase', letterSpacing: 0.7, marginBottom: 12, marginLeft: 4 },
  valueRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 14, backgroundColor: colors.mistWhite, borderRadius: 14, padding: 16, marginBottom: 10, ...shadow('#000', 0, 1, 4, 0.04, 1) },
  ...
  copyright: { fontFamily: fonts.regular, fontSize: 12, color: '#9CA3AF', textAlign: 'center', marginBottom: 8 },
})
