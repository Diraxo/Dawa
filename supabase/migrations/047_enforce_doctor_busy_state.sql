-- Migration: enforce_doctor_busy_state
--
-- Problem: book_appointment_slot() validates acceptOnDemand/acceptScheduled,
-- day-off, blocked_dates, and working hours (migrations 044/045) but never
-- checks whether the doctor is already inside another consultation. Neither
-- client (BookingModal.tsx on mobile, the web booking page) checks this
-- either — both gate "Start Now" purely on doctor_profiles.is_online and
-- acceptOnDemand. Result: a second patient can pay for and create an
-- On-Demand consultation with a doctor who already has one in progress.
--
-- Fix: a doctor is BUSY when they have a consultation with status 'accepted'
-- or 'in_progress'. For On-Demand bookings only, reject with DOCTOR_BUSY
-- before the consultation row (and therefore payment) is ever created.
-- Scheduled bookings are intentionally exempt — the doctor's current session
-- must not block a patient from booking a future slot.
--
-- Stale/zombie 'accepted'/'in_progress' rows are already bounded by
-- mark_stale_active_consultations() (migration 023/024), which flips them to
-- 'ended_abnormally' after a heartbeat timeout — so this check does not need
-- its own staleness window.

-- Lets clients (including anon/pre-auth booking screens) check busy state
-- without needing SELECT access to another patient's consultation row —
-- this returns only a boolean, no patient/consultation details.
CREATE OR REPLACE FUNCTION public.is_doctor_busy(p_doctor_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.is_doctor_busy(uuid) TO anon, authenticated;

-- CREATE OR REPLACE keeps the existing 9-param signature from migration 046 —
