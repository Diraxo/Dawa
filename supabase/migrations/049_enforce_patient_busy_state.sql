-- Migration: enforce_patient_busy_state
--
-- Problem: book_appointment_slot() enforces DOCTOR_BUSY (migration 047) so a
-- doctor can never be double-booked into two live consultations, but there is
-- no equivalent guard on the patient side. A patient can buy a second
-- On-Demand consultation while still 'waiting_for_doctor'/'accepted'/
-- 'in_progress' in an earlier one — neither client checks this either.
-- Symptom seen in production: a
-- patient bought a second chat consultation while the first was still open,
-- leaving the account with two "active" consultation states — one already
-- completed on the doctor's side, the other orphaned (its doctor never
-- actually accepted it) — which surfaced client-side as a permanent "Resume"
-- banner pointing at a dead session that could never be reached again.
--
-- Fix: mirror the DOCTOR_BUSY check for the patient, gated the same way —
-- On-Demand bookings only. A patient already mid-consultation must finish
-- (or have it end/decline/expire) before starting another one right now;
-- scheduling a future appointment while currently on a call is still
-- allowed, matching the doctor-side rationale in migration 047.

-- Lets clients check their own busy state before attempting to pay/book,
-- without needing a broader SELECT on the consultations table.
CREATE OR REPLACE FUNCTION public.is_patient_busy(p_patient_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.is_patient_busy(uuid) TO authenticated;

-- CREATE OR REPLACE keeps the existing 9-param signature from migration 047 —
