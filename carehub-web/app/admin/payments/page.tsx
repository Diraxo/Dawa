...
export default function AdminPaymentsPage() {
  ...
  useEffect(() => {
    supabase
      .from('consultations')
      .select('id, patient_amount, doctor_amount, platform_amount, payment_status, type, status, created_at, patient:users!patient_id(full_name), doctor_profile:doctor_profiles(user:users(full_name))')
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Total Revenue', value: `ETB ${totalRevenue.toLocaleString()}`, icon: '💰', color: 'from-int-blue to-teal-green' },
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-steel-grey">
              {loadingTx ? (
                ...
                <tr><td colSpan={7} className="text-center py-8 text-ink-black/40">No transactions yet</td></tr>
              ) : transactions.map(tx => (
                <tr key={tx.id} className="hover:bg-cloud-grey/50">
                  <td className="px-4 py-3 font-medium text-ink-black">{tx.patient?.full_name ?? '—'}</td>
                  <td className="px-4 py-3 text-ink-black/70">Dr. {tx.doctor_profile?.user?.full_name ?? '—'}</td>
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                      ...
                      : tx.status === 'active' ? 'bg-info/10 text-info'
            ...
            <tbody className="divide-y divide-steel-grey">
              {loadingWd ? (
                ...
                <tr key={wd.id} className="hover:bg-cloud-grey/50">
                  <td className="px-4 py-3 font-medium text-ink-black">Dr. {wd.doctor?.user?.full_name ?? '—'}</td>
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                      wd.status === 'approved' || wd.status === 'paid' ? 'bg-success/10 text-success'
      ...
      {tab === 'commission' && (
        <div className="card p-6 max-w-md">
          ...
          <p className="text-ink-black/50 text-sm mb-6">Percentage of each consultation fee kept by CareHub</p>
          ...
          <button
            onClick={() => {
              ...
              setTimeout(() => setSavingCommission(false), 1000)
            }}
            className="btn-primary w-full"
            disabled={savingCommission}
          >
            {savingCommission ? '✅ Saved!' : 'Save Commission Rate'}
          </button>
        </div>
      )}
    </div>
