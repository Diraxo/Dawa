...
export default function AppointmentsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data } = await client
        .from('consultations')
        .select('id, type, status, scheduled_at, created_at, patient_amount, doctor:doctor_profiles!doctor_id(specialty, user:users(full_name))')
  ...
  const filtered = appointments.filter(a => {
    if (tab === 'upcoming') return ['pending', 'active'].includes(a.status)
        ...
        <div className="flex flex-col gap-3">
          {filtered.map(a => (
            <div key={a.id} className="card p-5 flex items-center gap-4">
              {/* Doctor initial */}
              <div className="w-12 h-12 rounded-2xl bg-gradient-hero flex items-center justify-center text-white font-black text-base flex-shrink-0">
                {a.doctor?.user?.full_name?.charAt(0) ?? '?'}
              </div>
