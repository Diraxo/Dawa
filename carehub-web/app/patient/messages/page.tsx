...
export default function PatientMessagesPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Get all chat consultations for this patient
      const { data: consultations } = await client
        .from('consultations')
        .select('id, type, status, created_at, doctor:doctor_profiles!doctor_id(user:users(full_name))')
      ...
      // For each consultation, fetch its latest message
      const threadData: ChatThread[] = await Promise.all(
        (consultations as unknown as Array<{
          ...
          return {
            ...
            type: c.type,
            status: c.status,
            doctorName: c.doctor?.user?.full_name ?? 'Doctor',
