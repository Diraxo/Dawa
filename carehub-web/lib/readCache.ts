...
// If the user navigates from the chat page back to the messages list before
// that round trip finishes, `countUnread()` can still report the pre-read
// count. This module-level cache (survives client-side navigation since it's
// just a JS singleton, not component state) records the last message a
// channel was read through, so the messages list can deterministically show
// 0 unread for it regardless of how fast the user navigates back.
